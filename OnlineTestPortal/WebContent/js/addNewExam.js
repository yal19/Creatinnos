addNewExamNamespace.getHtmlSuccess = function(response){
	...
	addNewExamNamespace.loadData();
};

addNewExamNamespace.loadData = function(){
	$("#newCategory").empty();
	$('<option value=""> ---Select--- </option>').appendTo("#newCategory");
	for(var i=0;i<constants.category.length;i++){
		$('<option value='+ constants.category[i].cateId +'> '+ constants.category[i].categoryName +' </option>').appendTo("#newCategory");
	}
	...
	$category.change(function(){
		if($category.val() !==""){
			...
			for(var j=0;j<constants.subCategory.length;j++){
				if(constants.subCategory[j].Category === $category.val()) {
					$('<option value='+ constants.subCategory[j].subCateId +'> '+ constants.subCategory[j].subCategoryName +' </option>').appendTo($subCategory);
				}
			}
	...
	$subCategory.change(function(){
		...
		for(var m=0;m<constants.subject.length;m++){
			if(constants.subject[m].SubCategory === $subCategory.val()) {
				subAvail = true;
				$('<option value='+ constants.subject[m].subId +'> '+ constants.subject[m].SubjectName +' </option>').appendTo($subject);
			}
		}
		if(subAvail === false) {
...
addNewExamNamespace.pageEvents = function(){
	...
	$('#dpMode').off().click(function() {
		  ...
		  var examDetails = JSON.parse(localStorage.getItem("CRT_ExamDetails"));
			  if ($('#dpMode .switch_bg').hasClass('right')){   // Edit Mode
				  $("#saveExam").prop("disabled",false);
				  $("#dpMode.switch span:last").addClass('on');
				  
					$("#newExamName").val(examDetails[0].ExamName);
					$("#newExamStartDate").val(CommonNamespace.formatEventsCalDate(examDetails[0].ExamStartDate,"disp"));
					$("#newExamEndDate").val(CommonNamespace.formatEventsCalDate(examDetails[0].ExamEndDate,"disp"));
					$("#newExamHours").val(examDetails[0].ExamDurationHours);
					$("#newExamMin").val(examDetails[0].ExamDurationMin);
					$("#examTime").val(examDetails[0].ExamTime);
					$("#newPassPercent").val(examDetails[0].PassMark);
					if(examDetails[0].NegativeMark === "on"){
						$("#negativeMarks").prop("checked",true);
					}else{
						$("#negativeMarks").prop("checked",false);
					}
					var inst = examDetails[0].Instruction;
					$("#instSec").empty();
				  ...
				  // View Mode
					...
					$("#newNameLab").text(examDetails[0].ExamName);
					$("#newDateLab").text(CommonNamespace.formatEventsCalDate(examDetails[0].ExamStartDate,"disp"));
					$("#newEndDateLab").text(CommonNamespace.formatEventsCalDate(examDetails[0].ExamEndDate,"disp"));
					$("#newDurationLab").text(examDetails[0].ExamDurationHours + " Hours " + examDetails[0].ExamDurationMin + " Minutes ");
					$("#newTimeLab").text(examDetails[0].ExamTime);
					$("#newCandiLab").text($("#newCandidateType :selected").text().trim());
					$("#newAvailCandiLab").text($("#newCandidate :selected").text().trim());
					$("#newPassLab").text(examDetails[0].PassMark);
					if(examDetails[0].NegativeMark === "on"){
	...
	if(localStorage.getItem("CRT_AddNew") === "new"){
		$("#dpMode").hide();
		$(".editTabs").prop("disabled",true);
	}else{
		$("#dpMode").show();
		$(".editTabs").prop("disabled",false);
		var examId= localStorage.getItem("CRT_ExamID");
		var examDetails = constants.ExamInfo.filter(function(i,v){
			if(i.ExamId === examId){
				return i;
			}
		});
		$("#newCategory").val(examDetails[0].Category);
		$("#newCategory").change();
		$("#newSubcategory").change();
		$("#newSubject").val(examDetails[0].Subject);
		$("#newSubcategory").val(examDetails[0].SubCategory);
		$("#newCandidateType").val(examDetails[0].CandidatesType);
		$("#newCandidate").val(examDetails[0].Candidates);
		$('#dpMode .switch_bg').toggleClass('right');
		
		$("#cateQueryAll").text($("#newCategory :selected").text());
		$("#examNameAll").text(examDetails[0].ExamName);
		$("#examDateAll").text(CommonNamespace.formatEventsCalDate(examDetails[0].ExamStartDate,"disp") + " to " + CommonNamespace.formatEventsCalDate(examDetails[0].ExamEndDate,"disp") );
		$("#examDurationAll").text(examDetails[0].ExamDurationHours + " Hours " + examDetails[0].ExamDurationMin + " Minutes ");
		...
		localStorage.setItem("CRT_ExamDetails",JSON.stringify(examDetails));
...
addNewExamNamespace.saveNewExam  = function(){
	...
	var examStartDate = $("#newExamStartDate").val();
	var examEndDate = $("#newExamEndDate").val();
	...
	if(category !== "" && examName !== "" && examStartDate !== "" && examEndDate !== "" && examTime !== "" && (durationHours !=="" || durationMin !== "")){
		...
		var start = examStartDate.split("-");
		examStartDate = [start[2], start[1], start[0]].join("-");
		var end = examEndDate.split("-");
		examEndDate = [end[2], end[1], end[0]].join("-");
			var obj = {
				Category : category,
				...
				ExamDurationHours : durationHours,
